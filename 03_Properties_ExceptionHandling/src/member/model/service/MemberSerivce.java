...
public class MemberSerivce {
	...
	public List<Member> searchSearchName(String searchName) {
		Connection conn = getConnection();
		List<Member> list = memberDao.searchMemberName(conn, searchName);
		close(conn);
		return list;
	}
	...
	public Member memberLogin(String memberId, String password) {
		Connection conn = getConnection();
		Member member = memberDao.memberLogin(conn, memberId, password);
		close(conn);
		return member;
	}
	...
	public List<Member> deletMemberAll() {
		Connection conn = getConnection();
		List<Member> list = memberDao.deletMemberAll(conn);
		close(conn);
		return list;
	}
	...
	public int createTrigMemDel() {
		Connection conn = getConnection();
		int result = memberDao.createTrigMemDel(conn);
		if(result > 0) commit(conn);
		else rollback(conn);
		close(conn);
		return result;
